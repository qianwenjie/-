<template>
  <div class="favorite-list-page">
    <van-nav-bar title="收藏夹" left-arrow @click-left="$router.back()" fixed placeholder />

    <van-loading v-if="loading" class="loading-wrapper" vertical>加载中...</van-loading>

    <template v-else-if="favoriteItems.length > 0">
      <div class="question-list">
        ...
        >
          <div class="question-header">
            ...
            <span class="source-tag">{{ item.sourceTaskName }}</span>
          </div>
          <div class="question-content" v-html="item.question?.type === 'composite' ? item.question?.material : item.question?.content"></div>
          <div class="question-actions">
            <van-button size="small" plain type="warning" icon="star-o" @click.stop="removeFavorite(item.qId)">
              取消收藏
            </van-button>
          </div>
        </div>
    ...
    <van-empty v-else description="暂无收藏题目" image="search" />
      ...
      <div v-if="selectedItem" class="detail-popup">
        <div class="detail-header">
          <div class="header-info">
            ...
            <span class="source-badge">{{ selectedItem.sourceTaskName }}</span>
          </div>
        </div>
...
import { mockPracticeQuestionsMap } from '@/mock/practice'
...
// 构建题目映射：qId -> { question, sourceTaskId, sourceTaskName }
const taskNames = {
  pt001: '数据结构基础',
  pt002: '计算机网络冲刺',
  pt003: '操作系统原理',
...
onMounted(() => {
  store.loadFavorites()
  Object.entries(mockPracticeQuestionsMap).forEach(([taskId, questions]) => {
    questions.forEach(q => {
      allQuestions[q.id] = {
        question: q,
        sourceTaskId: taskId,
        sourceTaskName: taskNames[taskId] || taskId,
      }
    })
  })
  loading.value = false
